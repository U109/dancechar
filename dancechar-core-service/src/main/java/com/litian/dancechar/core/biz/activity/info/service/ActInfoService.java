...
import com.litian.dancechar.core.feign.idgen.enums.IdGenTypeEnum;
...
public class ActInfoService extends ServiceImpl<ActInfoDao, ActInfoDO> {
    ...
    public RespResult<String> createActInfo(ActInfoSaveDTO actInfoSaveDTO) {
        ...
        String actNo;
        RespResult<String> respResult = idGenClient.genId(IdGenReqDTO.builder().module(IdGenTypeEnum.ACT.getCode()).build());
        if(respResult.isOk()){
    ...
    public List<ActBaseInfoDTO> findActList(){
        return actInfoCacheService.findActListFromCache();
    }

    public ActBaseInfoDTO getByActNo(String actNo){
        return actInfoCacheService.getActInfoFromCache(actNo);
    }
