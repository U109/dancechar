...
public class RedisHelper {
    ...
    public List executeSyncPipeLinedSetString(List<Map.Entry<String, Object>> mapList, long seconds, TimeUnit timeUnit){
        RBatch batch = redissonClient.createBatch(BatchOptions.defaults());
        mapList.forEach((entry) ->{
            if(seconds >=0 && timeUnit != null){
                batch.getBucket(entry.getKey(), new StringCodec())
                        .setAsync((entry.getValue() instanceof String ? entry.getValue() :
                                JSONUtil.toJsonStr(entry.getValue())), seconds, timeUnit);
            }else {
                batch.getBucket(entry.getKey(), new StringCodec())
                        .setAsync((entry.getValue() instanceof String ? entry.getValue() :
    ...
    public void executeAsyncPipeLinedSetString(Map<String, Object> map, long seconds, TimeUnit timeUnit){
        RBatch batch = redissonClient.createBatch(BatchOptions.defaults());
