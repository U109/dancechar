...
public class DistributeLockAspect {
    ...
    public Object around(ProceedingJoinPoint point, Lock lock) throws Throwable{
        ...
        if(!lock.isBlock()){
            boolean isTryLock = ObjectUtil.equal(lock.expireTime(), -1L) ?
                    !distributedLock.tryLock(key, TimeUnit.MILLISECONDS, 0) :
                    !distributedLock.tryLock(key, TimeUnit.MILLISECONDS, 0, lock.expireTime());
            if(isTryLock){
            ...
            boolean isTryLock = ObjectUtil.equal(lock.expireTime(), -1L) ?
                    !distributedLock.tryLock(key, TimeUnit.MILLISECONDS, lock.waitTime()) :
                    !distributedLock.tryLock(key, TimeUnit.MILLISECONDS, lock.waitTime(), lock.expireTime());
            if(isTryLock){
                throw new DistributeLockException(StrUtil.isNotEmpty(lock.lockFailMsg())
                        ? lock.lockFailMsg() : LockConstant.OPERATE_QUICK);
            }
