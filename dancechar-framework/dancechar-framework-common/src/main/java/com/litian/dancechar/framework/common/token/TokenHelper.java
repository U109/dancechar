...
public class TokenHelper {
     ...
     * 生成admin token
     */
    public static String buildAdminToken(String userId, String accountNo) {
        return buildToken(userId, accountNo, 1, HttpConstants.Common.ADMIN_JWT_TOKEN_EXPIRE);
    }

    /**
     * 生成admin refresh token
     */
    public static String buildAdminRefreshToken(String userId, String accountNo) {
        return buildToken(userId, accountNo, 2, HttpConstants.Common.ADMIN_JWT_REFRESH_TOKEN_EXPIRE);
    }

    /**
     * 生成token
     */
    public static String buildAdminToken(String userId, String accountNo, Long expireMilliseconds) {
        return buildToken(userId, accountNo, 1, expireMilliseconds);
    }

    /**
     * 返回token
     */
    public static String buildToken(String userId, String accountNo, Integer type, Long expireMilliseconds) {
        Date now = new Date();
        ...
        return JWT.create().withIssuer(HttpConstants.Common.ISSUER).withIssuedAt(now)
                ...
                .withClaim(HttpConstants.Header.MOBILE, accountNo)
    ...
    public static Claims parse(String jsonWebToken, String base64Security) {
        try {
            ...
            log.error("执行JwtUtil.parse()方法,发生异常：{}", e.getMessage(), e);
            return null;
        }
    }
    ...
    public static Map<String,String> verify(String token){
        ...
        try {
            JWTVerifier verifier = JWT.require(Algorithm.HMAC256(HttpConstants.Common.JWT_SECRET)).build();
            verifier.verify(token);
            dataMap.put("isPass", "1");
            dataMap.put("expired", "0");
            return dataMap;
        } catch (Exception e){
            dataMap.put("isPass", "0");
            if(e instanceof com.auth0.jwt.exceptions.TokenExpiredException){
                // token过期
                dataMap.put("expired", "1");
            }
            return dataMap;
        }
    }
