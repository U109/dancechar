...
@Slf4j
@Component
public class ProduceCustomerGroupKafkaMsg implements InitializingBean, DisposableBean {
    @Resource
    ...
    public void sendMessage(String data) {
        ...
        if(!messageQueue.offer(data)){
            kafkaProducerUtil.sendMessage(CommConstants.KafkaTopic.TOPIC_CUSTOMER_GROUP_INFO, JSONUtil.toJsonStr(Lists.newArrayList(data)));
        }
    }
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        thread.setName("produce-customer-group-thread");
        thread.start();
    }
            ...
            try{
                while((msg = messageQueue.poll(5000L, TimeUnit.MILLISECONDS)) != null){
                    ...
                    if(messageList.size() >= BATCH_MESSAGE_LIST_SIZE || lengthSize >= BATCH_MESSAGE_LENGTH_SIZE){
                        ...
                        kafkaProducerUtil.sendMessage(CommConstants.KafkaTopic.TOPIC_CUSTOMER_GROUP_INFO, JSONUtil.toJsonStr(messageList));
                ...
                log.error("kafka批量发送消息系统异常！topic:{}",CommConstants.KafkaTopic.TOPIC_CUSTOMER_GROUP_INFO, e);
            }
        }
    }
}
